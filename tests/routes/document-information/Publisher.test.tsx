...
// Unmock the Publisher component to test the actual implementation
vi.unmock('../../../src/routes/document-information/Publisher')
  ...
  Textarea: ({
    ...
    return (
      <div data-testid="textarea-wrapper">
        <label data-testid="textarea-label" htmlFor={textareaId}>
          {label}
        </label>
        <textarea
          id={textareaId}
...
vi.mock('../../../src/components/forms/Select', () => ({
  default: ({
    ...
    const selectedValue =
      selectedKeys && selectedKeys.length > 0 ? String(selectedKeys[0]) : ''
...
describe('Publisher', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('should render all form fields with correct labels', () => {
      ...
      expect(
        screen.getByTestId('input-/document/publisher/name'),
      ).toBeInTheDocument()
      ...
      expect(
        screen.getByTestId('select-/document/publisher/category'),
      ).toBeInTheDocument()

      expect(inputLabels[1]).toHaveTextContent('Publisher Namespace')
      expect(
        screen.getByTestId('input-/document/publisher/namespace'),
      ).toBeInTheDocument()

      const textareaLabels = screen.getAllByTestId('textarea-label')
      expect(textareaLabels[0]).toHaveTextContent('Contact Details')
      expect(
        screen.getByTestId('textarea-/document/publisher/contact_details'),
      ).toBeInTheDocument()

      expect(textareaLabels[1]).toHaveTextContent('Issuing Authority')
      expect(
        screen.getByTestId('textarea-/document/publisher/issuing_authority'),
      ).toBeInTheDocument()
    })
    ...
    it('should render all publisher category options', () => {
      ...
      expect(
        screen.getByRole('option', { name: 'Discoverer' }),
      ).toBeInTheDocument()
      expect(screen.getByRole('option', { name: 'Other' })).toBeInTheDocument()
      expect(
        screen.getByRole('option', { name: 'Translator' }),
      ).toBeInTheDocument()
  ...
  describe('Form Field Attributes', () => {
    it('should set correct CSAF paths for all fields', () => {
      ...
      expect(
        screen.getByTestId('input-/document/publisher/name'),
      ).toHaveAttribute('data-csaf-path', '/document/publisher/name')
      expect(
        screen.getByTestId('select-/document/publisher/category'),
      ).toHaveAttribute('data-csaf-path', '/document/publisher/category')
      expect(
        screen.getByTestId('input-/document/publisher/namespace'),
      ).toHaveAttribute('data-csaf-path', '/document/publisher/namespace')
      expect(
        screen.getByTestId('textarea-/document/publisher/contact_details'),
      ).toHaveAttribute('data-csaf-path', '/document/publisher/contact_details')
      expect(
        screen.getByTestId('textarea-/document/publisher/issuing_authority'),
      ).toHaveAttribute(
        'data-csaf-path',
    ...
    it('should mark required fields correctly', () => {
      ...
      expect(
        screen.getByTestId('input-/document/publisher/name'),
      ).toHaveAttribute('data-is-required', 'true')
      expect(
        screen.getByTestId('select-/document/publisher/category'),
      ).toHaveAttribute('data-is-required', 'true')
      expect(
      ...
      expect(
        screen.getByTestId('textarea-/document/publisher/issuing_authority'),
      ).not.toHaveAttribute('data-is-required', 'true')
    })
  ...
  describe('User Interactions', () => {
    ...
    it('should update namespace when user types', async () => {
      ...
      const namespaceInput = screen.getByTestId(
        'input-/document/publisher/namespace',
      )
    ...
    it('should update contact details when user types', async () => {
      ...
      const contactDetailsTextarea = screen.getByTestId(
        'textarea-/document/publisher/contact_details',
      )
    ...
    it('should update issuing authority when user types', async () => {
      ...
      const issuingAuthorityTextarea = screen.getByTestId(
        'textarea-/document/publisher/issuing_authority',
      )
    ...
    it('should update category when user selects option', async () => {
      ...
      const categorySelect = screen.getByTestId(
        'select-/document/publisher/category',
      )
  ...
  describe('Page Visit Integration', () => {
    it('should render form fields with default page visit status', () => {
      ...
      expect(
        screen.getByTestId('input-/document/publisher/name'),
      ).toHaveAttribute('data-is-disabled', 'false')
      expect(
        screen.getByTestId('select-/document/publisher/category'),
      ).toHaveAttribute('data-is-disabled', 'false')
      expect(

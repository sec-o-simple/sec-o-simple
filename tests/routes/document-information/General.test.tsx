...
vi.mock('@/components/forms/Select', () => ({
  default: ({ label, selectedKeys, onSelectionChange, children, csafPath, className, isRequired, placeholder, renderValue }: any) => {
    const selectId = `select-${csafPath || 'select'}`
    const selectedValue = selectedKeys && selectedKeys.size > 0 ? String(Array.from(selectedKeys)[0]) : ''
...
vi.mock('@heroui/react', () => ({
  Alert: ({ children, color }: any) => <div data-testid="alert" data-color={color}>{children}</div>,
}))
...
describe('General', () => {
  ...
  describe('Component Rendering', () => {
    it('should render the wizard step with correct props', () => {
      ...
      expect(screen.getByTestId('wizard-title')).toHaveTextContent('General Information')
      expect(screen.getByTestId('progress')).toHaveAttribute('data-progress', '1')
      expect(screen.getByTestId('continue-link')).toHaveAttribute('href', '/document-information/notes')
    })
    ...
    it('should render all form fields', () => {
      ...
      // ID field
      expect(screen.getByTestId('input-/document/tracking/id')).toBeInTheDocument()
      ...
      expect(screen.getByTestId('select-/document/distribution/tlp/label')).toBeInTheDocument()
      expect(screen.getByTestId('input-/document/distribution/tlp/url')).toBeInTheDocument()
    })
  ...
  describe('Form Interactions', () => {
    ...
    it('should handle language selection changes', async () => {
      render(<General />)
      const languageSelect = screen.getByTestId('select-/document/lang') as HTMLSelectElement
    ...
    it('should handle TLP level selection changes', async () => {
      render(<General />)
      const tlpSelect = screen.getByTestId('select-/document/distribution/tlp/label') as HTMLSelectElement
    ...
    it('should handle TLP URL input changes', async () => {
      render(<General />)
      const tlpUrlInput = screen.getByTestId('input-/document/distribution/tlp/url')
      
      await user.type(tlpUrlInput, 'https://example.com/tlp')
      expect(tlpUrlInput).toHaveValue('https://example.com/tlp')
    })
  })
  ...
  describe('Field Properties', () => {
    it('should mark required fields as required', () => {
      ...
      const titleInput = screen.getByTestId('input-/document/title')
      const idInput = screen.getByTestId('input-/document/tracking/id')
      const languageSelect = screen.getByTestId('select-/document/lang')

      expect(titleInput).toHaveAttribute('data-is-required', 'true')
      expect(idInput).toHaveAttribute('data-is-required', 'true')
      expect(languageSelect).toHaveAttribute('data-is-required', 'true')
    })
    ...
    it('should set correct input types', () => {
      ...
      expect(tlpUrlInput).toHaveAttribute('type', 'url')
    })

    it('should apply CSAF paths correctly', () => {
      render(<General />)

      expect(screen.getByTestId('input-/document/title')).toHaveAttribute('data-csaf-path', '/document/title')
      expect(screen.getByTestId('input-/document/tracking/id')).toHaveAttribute('data-csaf-path', '/document/tracking/id')
      expect(screen.getByTestId('select-/document/lang')).toHaveAttribute('data-csaf-path', '/document/lang')
      expect(screen.getByTestId('select-/document/distribution/tlp/label')).toHaveAttribute('data-csaf-path', '/document/distribution/tlp/label')
      expect(screen.getByTestId('input-/document/distribution/tlp/url')).toHaveAttribute('data-csaf-path', '/document/distribution/tlp/url')
    })
  })
  ...
  describe('State Management', () => {
    it('should handle complex state updates for TLP object', async () => {
      ...
      // Update TLP URL - verify input works
      await user.type(tlpUrlInput, 'https://custom-tlp.com')
      expect(tlpUrlInput).toHaveValue('https://custom-tlp.com')
      
      // Verify both elements remain functional
      expect(tlpSelect).toBeInTheDocument()
      expect(tlpUrlInput).toBeInTheDocument()
    })
  ...
  describe('Edge Cases', () => {
    it('should handle empty/undefined state gracefully', () => {
      ...
      expect(screen.getByTestId('input-/document/distribution/tlp/url')).toHaveValue('')
    })
  ...
  describe('Layout and CSS Classes', () => {
    it('should apply correct CSS classes to layout components', () => {
      ...
      const languageSelectWrapper = screen.getByTestId('select-/document/lang').closest('[data-testid="select-wrapper"]')

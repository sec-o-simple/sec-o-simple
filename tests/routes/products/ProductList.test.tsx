...
describe('ProductList', () => {
  ...
  describe('Basic rendering', () => {
    it('should render container with correct classes', () => {
      ...
      expect(productListContainer).toHaveClass(
        'flex',
        'flex-col',
        'items-stretch',
        'gap-2',
  ...
  describe('Empty state', () => {
    ...
    it('should apply correct styling to empty message', () => {
      ...
      const emptyContainer = screen
        .getByText('No Software available')
        .closest('div')
  ...
  describe('Products display', () => {
    it('should render ProductCard for each matching software product', () => {
      ...
      expect(
        screen.getByText('ProductCard: Software Product One (Software)'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('ProductCard: Software Product Two (Software)'),
      ).toBeInTheDocument()

      // Hardware product should not be shown
      expect(
        screen.queryByText('ProductCard: Hardware Product One (Hardware)'),
      ).not.toBeInTheDocument()

      // Product without type should not be shown
      expect(
        screen.queryByText('ProductCard: Product Without Type'),
      ).not.toBeInTheDocument()
    })
    ...
    it('should render ProductCard for each matching hardware product', () => {
      ...
      expect(
        screen.getByText('ProductCard: Hardware Product One (Hardware)'),
      ).toBeInTheDocument()

      // Software products should not be shown
      expect(
        screen.queryByText('ProductCard: Software Product One (Software)'),
      ).not.toBeInTheDocument()
      expect(
        screen.queryByText('ProductCard: Software Product Two (Software)'),
      ).not.toBeInTheDocument()
    })
    ...
    it('should not show empty message when products are available', () => {
      ...
      expect(
        screen.queryByText('No Software available'),
      ).not.toBeInTheDocument()
  ...
  describe('Filtering logic', () => {
    it('should filter products by exact type match', () => {
      ...
      expect(
        screen.getByText('ProductCard: Test Product (Software)'),
      ).toBeInTheDocument()
    })
    ...
    it('should handle products without type property', () => {
      ...
      expect(productCards).toHaveLength(1)
      expect(
        screen.getByText('ProductCard: Product With Type (Software)'),
      ).toBeInTheDocument()
      expect(
        screen.queryByText('ProductCard: Product No Type'),
      ).not.toBeInTheDocument()
    })
    ...
    it('should handle undefined type values', () => {
      ...
      expect(productCards).toHaveLength(1)
      expect(
        screen.getByText('ProductCard: Software Product (Software)'),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('Edge cases', () => {
    ...
    it('should handle large number of products', () => {
      const manyProducts: TProductTreeBranch[] = Array.from(
        { length: 100 },
        (_, i) => ({
          id: `product-${i}`,
          category: 'product_name',
    ...
    it('should handle products with special characters in names', () => {
      ...
      expect(
        screen.getByText('ProductCard: Product @#$%^&*() (Software)'),
      ).toBeInTheDocument()
      expect(
        screen.getByText(
          'ProductCard: Product with "quotes" and <tags> (Software)',
        ),
      ).toBeInTheDocument()
    })

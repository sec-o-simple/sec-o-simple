...
vi.mock('@heroui/theme', () => ({
  cn: (...classes: any[]) => {
    ...
    return classes
      .filter(cls => typeof cls === 'string' || (typeof cls === 'object' && cls !== null))
      .map(cls => {
...
describe('InfoCard', () => {
  describe('Basic Functionality', () => {
    ...
    it('should pass through HTML div props', () => {
      ...
      expect(container).toHaveAttribute('class', expect.stringContaining('test-class'))
  ...
  describe('Variant Handling', () => {
    it('should render plain variant by default', () => {
      ...
      expect(screen.queryByText('Test Title')?.closest('.rounded-lg')).not.toBeInTheDocument()
      expect(screen.getByTestId('vsplit')).toBeInTheDocument()
    })
    ...
    it('should render boxed variant', () => {
      ...
      const boxedContainer = screen.getByText('Test Title').closest('.rounded-lg')
      expect(boxedContainer).toBeInTheDocument()
      expect(boxedContainer).toHaveClass('border', 'bg-content1', 'px-4', 'py-2')
    })
  ...
  describe('Link Functionality', () => {
    ...
    it('should render title as link when linkTo prop is provided', () => {
      renderWithRouter(
        <InfoCard title="Linked Title" linkTo="/test-path" />
      )
    ...
    it('should apply hover:underline class when linkTo is provided', () => {
      ...
      const titleContainer = screen.getByText('Linked Title').closest('.font-bold')
    ...
    it('should not apply hover:underline class when no linkTo', () => {
      ...
      const titleContainer = screen.getByText('Plain Title').closest('.font-bold')
  ...
  describe('Complex Scenarios', () => {
    it('should render all props together in plain variant', () => {
      ...
      expect(screen.getByRole('link', { name: 'Complex Title' })).toHaveAttribute('href', '/complex-path')
      ...
      expect(container).toHaveAttribute('class', expect.stringContaining('custom-class'))
    ...
    it('should render all props together in boxed variant', () => {
      ...
      const boxedContainer = screen.getByText('Boxed Complex Title').closest('.rounded-lg')
      expect(boxedContainer).toBeInTheDocument()
      ...
      expect(screen.getByRole('link', { name: 'Boxed Complex Title' })).toHaveAttribute('href', '/boxed-path')
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty description', () => {
      ...
      const descriptionElement = document.querySelector('.text-neutral-foreground')
  ...
  describe('CSS Classes and Styling', () => {
    it('should apply correct CSS classes to title container', () => {
      ...
      const titleContainer = screen.getByText('Styled Title').closest('.font-bold')
      expect(titleContainer).toHaveClass('font-bold')
    })
    ...
    it('should apply correct CSS classes to title container with link', () => {
      ...
      const titleContainer = screen.getByText('Linked Styled Title').closest('.font-bold')
      expect(titleContainer).toHaveClass('font-bold', 'hover:underline')
    })

    it('should apply correct CSS classes to description', () => {
      renderWithRouter(<InfoCard title="Title" description="Styled description" />)
    ...
    it('should apply correct CSS classes to content areas', () => {
      ...
      const rightContentArea = leftContentArea?.parentElement?.querySelector('div:last-child')

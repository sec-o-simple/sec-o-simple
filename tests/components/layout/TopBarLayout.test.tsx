...
vi.mock('@heroui/react', () => ({
  ToastProvider: vi.fn(() => <div data-testid="toast-provider">Toast Provider</div>),
  Button: vi.fn(({ children, onPress, isDisabled, color, variant, className }: any) => (
    <button 
      onClick={onPress}
      disabled={isDisabled}
      className={className}
  ...
  ModalContent: vi.fn(({ children }: any) => <div data-testid="modal-content">{children}</div>),
  ...
  Table: vi.fn(({ children }: any) => <table data-testid="table">{children}</table>),
  TableHeader: vi.fn(({ children }: any) => <thead data-testid="table-header">{children}</thead>),
  TableBody: vi.fn(({ children }: any) => <tbody data-testid="table-body">{children}</tbody>),
  TableColumn: vi.fn(({ children }: any) => <th data-testid="table-column">{children}</th>),
  TableRow: vi.fn(({ children }: any) => <tr data-testid="table-row">{children}</tr>),
  TableCell: vi.fn(({ children }: any) => <td data-testid="table-cell">{children}</td>),
  useDisclosure: vi.fn(() => ({
    isOpen: false,
    onOpen: vi.fn(),
    onClose: vi.fn(),
...
vi.mock('../../../src/components/forms/ConfirmButton', () => ({
  default: vi.fn(({ children, onConfirm, className, color, fullWidth }: any) => (
    <button 
      onClick={onConfirm}
      className={className}
...
vi.mock('@fortawesome/react-fontawesome', () => ({
  FontAwesomeIcon: vi.fn(({ icon }: any) => <span data-testid="font-awesome-icon" data-icon={icon?.iconName}>Icon</span>),
}))

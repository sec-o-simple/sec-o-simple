 ...
 * ```
 */
export function retrieveLatestVersion(
  history: TRevisionHistoryEntry[],
): string {
  if (history.length === 0) {
    throw new Error(
      'Revision history is empty, cannot retrieve latest version.',
    )
  }
  ...
  const sorted = history
    ...
    .sort(
      (a, z) =>
        compareZonedDateTimes(new Date(z.date), new Date(a.date)) ||
